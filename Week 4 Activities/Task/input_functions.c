...
{
	printf("%s", prompt);
	//scanf("%s", destination); This does not read white space
	//The one below does, using RegEx
	scanf("%[^\n]", destination); //[^\n] means read every character (including white space) until new line (enter key) is pressed
	flush();
}
